...
const dataFiltered = (which, args) => {
    let result = database[which].filter((item) => {
        /* 조건 인자가 없거나, 페이징 관련 인자거나, 모든 요소가 아이템과 모두 일치하면 통과 */
...
const dbWorks = {
    ...
    getTeams: (args) => dataFiltered('teams', args).map((team) => {
        team.members = dbWorks.getPeople({ team: team.id })
        return team
    }),
    postTeam: (args) => {
        const newTeam = {
            id: database.teams.map((team) => Number(team.id)).reduce((a, b) => Math.max(a, b), 0) + 1,
            ...args.input
        }
    ...
    editTeam: (args) => {
        return database.teams.filter((team) => team.id == args.id).map((team) => {
            Object.assign(team, args.input)
            return team
        })[0]
    },

    getPeople: (args) => dataFiltered('people', args).map((person) => {
        person.tools = [
    ...
    postPerson: (args) => {
        const newPerson = {
            id: database.people.map((person) => Number(person.id)).reduce((a, b) => Math.max(a, b), 0) + 1,
            ...args.input
        }
    ...
    editPerson: (args) => {
        return database.people.filter((person) => person.id == args.id).map((person) => {
            Object.assign(person, args.input)
            return person
        })[0]
    },

    getRoles: (args) => dataFiltered('roles', args).map((role) => {
    ...
    increaseEquipment: (args) => {
        return database.equipments.filter((equipment) => equipment.id == args.id).map((equipment) => {
            equipment.count += 1
            return equipment
        })[0]
    },
